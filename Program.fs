...
let registerUser userInput =
    ...
    if registeredUsersMap.ContainsKey userInput.userID then
        ...
        response <- rectype |> Json.toJson |> System.Text.Encoding.UTF8.GetString
    else
        registeredUsersMap <- Map.add userInput.userID userInput.value registeredUsersMap
...
let loginUser userInput =
  let mutable resp = ""
  if registeredUsersMap.ContainsKey userInput.userID then
    if registeredUsersMap.[userInput.userID] = userInput.value then
